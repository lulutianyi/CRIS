...
class ReverseModel(nn.Module):
    """
    Predicts prob(z0=1) given (z_t, t).
    Lightweight MLP — swap for UNet for better quality.
    D = 3*32*32 = 3072 for CIFAR-10 pixel binarization.
    """
    def __init__(self, D=3072, T=T_MAX):
        super().__init__()
    ...
    def forward(self, zt, t):
        t  = t.clamp(0, self.t_emb.num_embeddings - 1)
        te = self.t_emb(t)                               # (B, 64)
        h  = torch.cat([zt.float(), te], dim=-1)
        return self.net(h)                               # logits (B, D)
...
def main():
    ...
    model = ReverseModel(D=3*32*32, T=T_MAX).to(DEVICE)
    opt   = torch.optim.Adam(model.parameters(), lr=LR)

    import os
    if os.path.exists("purify_v2.pt"):
        print("\nLoading existing checkpoint purify_v2.pt ...")
        model.load_state_dict(torch.load("purify_v2.pt", map_location=DEVICE))
    else:
        print("\nTraining reverse model on clean CIFAR-10...")
        for ep in range(EPOCHS):
            loss = train_epoch(model, train_loader, betas, flip_prob, opt)
            print(f"  Epoch {ep+1}/{EPOCHS}  loss={loss:.4f}")
        torch.save(model.state_dict(), "purify_v2.pt")
